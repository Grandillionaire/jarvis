# Free APIs Jarvis can use

When the user wants live info, call these via `curl` (Bash tool). Keys (4 of them) live in
`~/.claude/jarvis/api-keys.env` — read that file for the value; never print keys aloud.
...
- **Knowledge — Wikipedia:** `curl -H 'User-Agent: Jarvis/1.0' 'https://en.wikipedia.org/api/rest_v1/page/summary/<Topic>'`
- **FX rates — Frankfurter:** `curl 'https://api.frankfurter.dev/v1/latest?base=EUR&symbols=USD'` (ECB rates, no quota).
- **Geocoding — Nominatim:** `curl -H 'User-Agent: Jarvis/1.0' 'https://nominatim.openstreetmap.org/search?q=London&format=json&limit=1'` (max 1 req/sec).
- **Stocks (quick) — Stooq:** `curl 'https://stooq.com/q/l/?s=aapl.us&f=sd2t2ohlcv&h&e=csv'`

## Free key needed (from ~/.claude/jarvis/api-keys.env)
...
To load keys in a shell call: `set -a; . ~/.claude/jarvis/api-keys.env; set +a` then curl.
