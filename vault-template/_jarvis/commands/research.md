...
1. **Research deeply.** Use the web (Tavily search per `_jarvis/apis.md`, plus Wikipedia and any
...
2. **Write it up as a document** in `03_Resources/research-<kebab-topic>.md`, built from
   `99_Templates/research.md`. **Keep the `![[jarvis-logo.svg|90]]` embed as the very first line**
   (the Jarvis logo, top-left), then fill in Summary → Key findings → Details → Sources → Open
...
3. **Open it in Obsidian** so the user can read along: use the Obsidian `open_file` tool on the new note
   (fallback: `open "obsidian://open?vault=Jarvis&file=03_Resources/research-<kebab-topic>"`).
