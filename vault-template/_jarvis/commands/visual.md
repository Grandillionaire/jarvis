...
2. **If data is missing**, pull it from the relevant source (the vault, a calendar, an API in `_jarvis/apis.md`)
...
3. **Save + embed + open.** Put the visual in `03_Resources/visuals/`; for a chart/diagram, create a note
   `03_Resources/visuals/<name>.md` that starts with `![[jarvis-logo.svg|90]]`, a title, and embeds the
