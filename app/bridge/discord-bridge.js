'use strict';
// Discord bridge — owner-allowlisted DM control of Jarvis via the Gateway (built-in WebSocket, no deps).
...
function onMessage(p) {
  ...
  if (p.op === 10) { // hello -> heartbeat + identify
    ...
    send({ op: 2, d: { token: TOKEN, intents: INTENTS, properties: { os: 'mac', browser: 'jarvis', device: 'jarvis' } } });
...
async function main() {
  ...
  if (!TOKEN || !OWNER) { console.error('discord-bridge: set DISCORD_BOT_TOKEN and DISCORD_OWNER_USER_ID in ~/.claude/jarvis/bridge.env'); process.exit(1); }
