...
- **Phone control.** Drive it from Telegram, Discord, Slack, iMessage, or Email (draft-only) — owner-allowlisted, text or **voice memos** (transcribed locally on your Mac, never by a cloud STT). Remote turns are sandboxed and read-only by default — a message can read and search your vault, but can't write files, run shell, fetch the network, or touch your machine (web lookup and capture are opt-in).
